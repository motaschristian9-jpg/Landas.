...
export default function Index({ todos, showingHistory }) {
    ...
    const slots = [
        { id: 'morning', icon: <Sun size={18} />, label: 'Morning Sprint', color: 'text-amber-500' },
        { id: 'afternoon', icon: <Sunset size={18} />, label: 'Deep Work', color: 'text-orange-500' },
        { id: 'evening', icon: <Moon size={18} />, label: 'Wind Down', color: 'text-indigo-500' },
    ];
    ...
    return (
        <AuthenticatedLayout>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-12">
                    <div>
                        <div className="flex items-center space-x-4 mb-3 opacity-40">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.5em] text-slate-900">Pillar One</span>
                        </div>
                        <h1 className="text-7xl font-black tracking-tighter text-slate-900 leading-[0.8] mb-6">
                            {showingHistory ? <>Mastery <span className="text-emerald-500">History.</span></> : <>Daily <span className="text-emerald-500">Sprint.</span></>}
                        </h1>
                        <p className="text-slate-400 font-bold text-lg">
                            {showingHistory ? "Your lifetime of wins, preserved." : "Turning intentions into action."}
                        </p>
                    </div>
                    <div className="mt-8 md:mt-0 flex items-center space-x-4">
                        ...
                        >
                            <Clock size={14} />
                            <span>{showingHistory ? 'View Sprint' : 'View History'}</span>
                        </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-4 space-y-8">
                            ...
                            <div className="relative z-10">
                                ...
                                {focusTask ? (
                                    <>
                                        ...
                                        <div className="flex items-center space-x-6 mb-8">
                                            <div className="flex items-center space-x-2 text-slate-400 text-xs font-bold">
                                                <Timer size={14} />
                                                <span>{focusTask.estimated_minutes || 30}m</span>
                                            </div>
                                            <div className="flex items-center space-x-2 text-slate-400 text-xs font-bold">
                                    ...
                                    <div className="py-10 text-center">
                                        <p className="text-slate-500 font-bold italic">No active focus. Add a task to start.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[3rem] p-8 border-2 border-slate-50 shadow-xl shadow-slate-100/50">
                            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Daily Momentum</h3>
                    ...
                    <div className="lg:col-span-8 flex flex-col">
                        ...
                        <div className="space-y-12">
                            {slots.map(slot => {
                                const slotTasks = filteredTodos.filter(t => t.time_slot === slot.id || (!t.time_slot && slot.id === 'morning'));
                                if (slotTasks.length === 0 && filter !== 'all') return null;
                                ...
                                return (
                                    <div key={slot.id} className="relative pl-12">
                                        ...
                                        <div className="mb-6 pt-2">
                                            <h4 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">{slot.label}</h4>
                                        </div>
                                        ...
                                        <div className="space-y-4">
                                                ...
                                                {slotTasks.map(todo => (
                                                    ...
                                                    >
                                                        <div className="flex items-center space-x-6">
                                                            <div className={todo.is_completed ? 'text-emerald-500' : 'text-slate-200'}>
                                                                {todo.is_completed ? <CheckCircle2 size={28} strokeWidth={3} /> : <Circle size={28} strokeWidth={2.5} />}
                                                            </div>
                                                            <div>
                                                                <h4 className={`text-xl font-black tracking-tight ${todo.is_completed ? 'line-through text-slate-400' : 'text-slate-800'}`}>
                                                                    {todo.title}
                                                                </h4>
                                            ...
                                            {slotTasks.length === 0 && (
                                                <div className="py-8 border-2 border-dashed border-slate-50 rounded-[2.5rem] flex items-center justify-center">
                                                    <p className="text-[10px] font-black text-slate-200 uppercase tracking-widest">No tasks scheduled</p>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    >
                        ...
                        <form onSubmit={submit} className="space-y-6">
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">What's the move?</label>
                                <input 
                                    autoFocus
                                    type="text" 
                                    required 
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Energy Slot</label>
                                    <div className="flex space-x-2">
                                        {['morning', 'afternoon', 'evening'].map(s => (
                                            <button
                                                key={s}
                                                type="button"
                                                onClick={() => setData('time_slot', s)}
                                            ...
                                            >
                                                {s === 'morning' ? <Sun size={18} /> : (s === 'afternoon' ? <Sunset size={18} /> : <Moon size={18} />)}
                                            </button>
                                        ))}
                                    </div>
                                </div>
            ...
            <ConfirmationModal 
                ...
                title="Delete Action?"
                message={todoToDelete ? `Are you sure you want to delete "${todoToDelete.title}"? This cannot be undone.` : ''}
